...
export const getShipmentForQC = async (req, res) => {
  try {
    ...
    const shipment = await IncomingShipment.findById(id)
      .populate('gateEntry.vendor', 'name')
      .populate('gateEntry.enteredBy', 'name')
      .populate('gateEntry.securityCountedBy', 'name');
...
// Submit QC inspection - PASS (send to weighbridge)
export const submitQCPass = async (req, res) => {
  try {
    ...
    // Update status - move to weighbridge (or next stage)
    ...
    res.status(200).json({
      success: true,
      message: "QC inspection passed. Shipment sent to weighbridge.",
      data: updatedShipment
    });
  } catch (error) {
    res.status(500).json({
